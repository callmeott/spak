...
public class ScriptEngine extends Thread {
  ...
  private void threadMain() {
    processOneLine("importPackage(Packages.spak)");
    while(!stopme) {
      String cmd=getCommand();
      if(cmd.startsWith("file:"))
        processSource(cmd.substring(5));
      else
        processOneLine(cmd);
    }
  }
  ...
  /**
   * Wait until Script Thread finishes the ongoing command
   */
  ...
  public String getErrorString() {
    String tmp = errorString.toString();
    errorString = "";
    return tmp;
  }
  ...
  private synchronized String getCommand() {
    while(commandList.size()<=0) {
      busyFlag=false;
      notifyAll();  // Release WaitDone()
      try {
        wait();
      }
      catch(Exception e){ System.err.println("Error:"+e);}
    }
    String cmd = (String)commandList.elementAt(0);
    commandList.removeElementAt(0);
    System.out.println("Evaluate: "+cmd);
    return cmd;
  }
  ...
  public synchronized void putSource(String str) {
    busyFlag=true;
    commandList.add("file:"+str);
    notifyAll();
  }

  public static void putVar(String name, Scriptable obj) {
    global.put(name,global,obj);
  }

  public static void putVar(String name, KFrame fr) {
    global.put(name,global,cx.toObject(fr, global));
  }

  void setShell(JScriptShell j) {
    jss=j;
  }
  ...
  public static int exec(String args[]) {
    ...
    // Define root object
    Scriptable rootobj = Context.toObject(root, global);
    putVar("Root", rootobj);
    // global.put("r",global,rootobj);
    putVar("r",root.getKFrameScript());

    Scriptable wmobj = Context.toObject(root.getWorkingMemory(), global);
    putVar("wm", wmobj);
   ...
   */
  public static void processSource(String filename) {
...
//        System.gc();
                                      ...
                                      int lineno)
  {
    ...
    try {
      ...
      p(we.getWrappedException().toString()+"\n");
      errorString = we.getWrappedException().toString();
...
//      }
  ...
  private static void p(String s) {
    if(jss!=null)
      jss.p(s);
    else // Use STDERR in case no JS Shell attached
      System.err.print("JS: "+s);
  }
  ...
  public void processOneLine(String source) {
    // Use the interpreter for interactive input
    cx.setOptimizationLevel(-1);
    source = source + "\n";
    if (cx.stringIsCompilableUnit(source)) {
